...
export type FivemActionSettings = { id: string; botId: string; guildId: string; architecture: FivemActionArchitecture; enabled: boolean; categoryId: string | null; panelChannelId: string | null; actionChannelId: string | null; reportChannelId: string | null; managerRoleIds: string[]; reportBannerUrls: string[]; spreadsheetEnabled: boolean; spreadsheetId: string | null; spreadsheetSheetName: string | null; spreadsheetLastSyncAt: string | null; spreadsheetSyncError: string | null; panelMessageId: string | null; panelTitle: string; panelDescription: string; color: string; imageUrl: string | null; imagePosition: "top" | "center" | "bottom" | "none"; lastPanelRequestedAt: string | null; updatedAt: string };
...
export type PoliceQruOfficer = { id: string; mention: string; name: string };
export type PoliceQruSettings = { id: string; botId: string; guildId: string; enabled: boolean; recordChannelId: string | null; logChannelId: string | null; approvalChannelId: string | null; temporaryCategoryId: string | null; allowedRoleIds: string[]; supervisorRoleIds: string[]; teamRoleId: string | null; deleteChannelSeconds: number; color: string; panelTitle: string; panelDescription: string; panelImageUrl: string | null; panelMessage: string; rankingChannelId: string | null; rankingMessageId: string | null };
export type PoliceQruRecord = { id: string; botId: string; guildId: string; boNumber: string; qruType: string; occurrenceDate: string; evidenceUrl: string; seizures: string | null; notes: string | null; vehicle: string | null; status: "pending" | "approved" | "rejected"; authorId: string; authorName: string; officers: PoliceQruOfficer[]; temporaryChannelId: string | null; recordChannelId: string | null; recordMessageId: string | null; approvalChannelId: string | null; approvalMessageId: string | null; approvedById: string | null; approvedByName: string | null; approvedAt: string | null; rejectionCount: number; rejections: Array<{ reason: string; rejectedAt: string; supervisorId: string; supervisorName: string }>; createdAt: string; updatedAt: string };
export type PoliceQruRankingEntry = { firstQruAt: string | null; lastQruAt: string | null; officerId: string; officerName: string; position: number; total: number };
export type PoliceQruProfile = { firstQruAt: string | null; lastQruAt: string | null; officerId: string; officerName: string | null; position: number | null; registeredBos: number; total: number };
export type PolicePromotionQuestionType = "short" | "paragraph" | "number" | "date" | "time" | "select" | "checkbox" | "radio";
export type PolicePromotionQuestion = { id: string; active: boolean; defaultValue: string | null; description: string | null; label: string; maxLength: number | null; options: string[]; order: number; placeholder: string | null; required: boolean; type: PolicePromotionQuestionType };
export type PolicePromotionDefinition = { id: string; active: boolean; approvalRoleIds: string[]; categoryId: string | null; color: string; description: string; emoji: string | null; evaluatorRoleIds: string[]; grantedRoleId: string | null; historyChannelId: string | null; logChannelId: string | null; name: string; panelChannelId: string | null; panelDescription: string; panelMessageId: string | null; panelTitle: string; receivedRankName: string; rejectedRoleIds: string[]; removedRoleId: string | null; requestNewEvaluationEnabled: boolean; questions: PolicePromotionQuestion[] };
export type PolicePromotionSettings = { id: string; botId: string; guildId: string; enabled: boolean; defaultApprovalChannelId: string | null; defaultCategoryId: string | null; defaultHistoryChannelId: string | null; defaultLogChannelId: string | null; defaultPanelChannelId: string | null; promotions: PolicePromotionDefinition[]; createdAt: string; updatedAt: string };
export type PolicePromotionAnswer = { questionId: string; label: string; type: PolicePromotionQuestionType; value: string | string[] };
export type PolicePromotionRequest = { id: string; approvalChannelId: string | null; approvalMessageId: string | null; approvalReason: string | null; approvalResult: "approved" | "rejected" | null; approvedAt: string | null; approvedById: string | null; approvedByName: string | null; answers: PolicePromotionAnswer[]; botId: string; channelId: string | null; channelMessageId: string | null; createdAt: string; currentRank: string; evaluationAttemptNumber: number | null; evaluationEndedAt: string | null; evaluationNotes: string | null; evaluationResult: "approved" | "rejected" | null; evaluationStartedAt: string | null; evaluatorId: string | null; evaluatorName: string | null; guildId: string; history: Array<{ action: string; actorId: string | null; actorName: string | null; at: string; metadata: Record<string, unknown> }>; inGameId: string; logChannelId: string | null; previousRequestId: string | null; promotionId: string; promotionName: string; requesterId: string; requesterName: string; requestedDate: string; requestedTime: string; status: "submitted" | "ticket_open" | "in_evaluation" | "pending_approval" | "approved" | "rejected" | "cancelled" | "closed"; targetRank: string; updatedAt: string };
export type PolicePromotionEvaluationHistory = { cadetId: string; cadetName: string; evaluatedAt: string; evaluationId: string; evaluationNotes: string | null; evaluationType: string; evaluatorId: string | null; evaluatorName: string | null; rejectionReason: string | null; result: "approved" | "rejected"; attemptNumber: number };
export type PolicePromotionEvaluationHistoryPage = { records: PolicePromotionEvaluationHistory[]; total: number };
...
export class ApiClient {
  ...
  async getVehicleAbandonmentSettings(guildId: string) { const { data } = await this.http.get<{ settings: VehicleAbandonmentSettings }>(`/vehicle-abandonment/bot/${guildId}/settings`); return data.settings; }
  async createVehicleAbandonmentRecord(input: { authorId: string; authorName: string; guildId: string; imageUrls: string[]; model: string; plate: string; recordChannelId: string; recordMessageId?: string | null; report: string; sourceMessageId: string; systemChannelId: string }) { const { data } = await this.http.post<{ record: VehicleAbandonmentRecord }>("/vehicle-abandonment/bot/records", input); return data.record; }
  async getVehicleAbandonmentRecord(recordId: string) { const { data } = await this.http.get<{ record: VehicleAbandonmentRecord }>(`/vehicle-abandonment/bot/records/${recordId}`); return data.record; }
  async updateVehicleAbandonmentRecord(recordId: string, input: Partial<Pick<VehicleAbandonmentRecord, "model" | "plate" | "recordMessageId" | "report">>) { const { data } = await this.http.patch<{ record: VehicleAbandonmentRecord }>(`/vehicle-abandonment/bot/records/${recordId}`, input); return data.record; }
  async getPoliceQruSettings(guildId: string) { const { data } = await this.http.get<{ settings: PoliceQruSettings }>(`/police-qru/bot/${guildId}/settings`); return data.settings; }
  async savePoliceQruSettings(guildId: string, input: Partial<Pick<PoliceQruSettings, "rankingChannelId" | "rankingMessageId">>) { const { data } = await this.http.patch<{ settings: PoliceQruSettings }>(`/police-qru/bot/${guildId}/settings`, input); return data.settings; }
  async createPoliceQruRecord(input: { approvalChannelId?: string | null; approvalMessageId?: string | null; authorId: string; authorName: string; boNumber: string; evidenceUrl: string; guildId: string; notes?: string | null; occurrenceDate: string; officers: PoliceQruOfficer[]; qruType: string; recordChannelId?: string | null; recordMessageId?: string | null; seizures?: string | null; status?: "pending" | "approved"; temporaryChannelId?: string | null; vehicle: string }) { const { data } = await this.http.post<{ record: PoliceQruRecord }>("/police-qru/bot/records", input); return data.record; }
  async resubmitPoliceQruRecord(recordId: string, input: { approvalChannelId?: string | null; approvalMessageId?: string | null; authorId: string; authorName: string; boNumber: string; evidenceUrl: string; guildId: string; notes?: string | null; occurrenceDate: string; officers: PoliceQruOfficer[]; qruType: string; recordChannelId?: string | null; recordMessageId?: string | null; seizures?: string | null; temporaryChannelId?: string | null; vehicle: string }) { const { data } = await this.http.put<{ record: PoliceQruRecord }>(`/police-qru/bot/records/${recordId}`, input); return data.record; }
  async updatePoliceQruApprovalMessage(recordId: string, input: { approvalChannelId?: string | null; approvalMessageId?: string | null }) { const { data } = await this.http.patch<{ record: PoliceQruRecord }>(`/police-qru/bot/records/${recordId}/approval-message`, input); return data.record; }
  async approvePoliceQruRecord(recordId: string, input: { supervisorId: string; supervisorName: string }) { const { data } = await this.http.post<{ record: PoliceQruRecord }>(`/police-qru/bot/records/${recordId}/approve`, input); return data.record; }
  async rejectPoliceQruRecord(recordId: string, input: { reason: string; supervisorId: string; supervisorName: string }) { const { data } = await this.http.post<{ record: PoliceQruRecord }>(`/police-qru/bot/records/${recordId}/reject`, input); return data.record; }
  async updatePoliceQruRecordMessage(recordId: string, input: { recordChannelId?: string | null; recordMessageId?: string | null }) { const { data } = await this.http.patch<{ record: PoliceQruRecord }>(`/police-qru/bot/records/${recordId}/message`, input); return data.record; }
  async getPoliceQruRanking(guildId: string, limit = 20) { const { data } = await this.http.get<{ ranking: PoliceQruRankingEntry[] }>(`/police-qru/bot/${guildId}/ranking`, { params: { limit } }); return data.ranking; }
  async getPoliceQruProfile(guildId: string, officerId: string) { const { data } = await this.http.get<{ profile: PoliceQruProfile }>(`/police-qru/bot/${guildId}/profile/${officerId}`); return data.profile; }
  async searchPoliceQruRecords(guildId: string, params: { authorId?: string | null; boNumber?: string | null; occurrenceDate?: string | null; officerId?: string | null; qruType?: string | null; limit?: number }) { const { data } = await this.http.get<{ records: PoliceQruRecord[] }>(`/police-qru/bot/${guildId}/records`, { params }); return data.records; }
  async createPoliceQruLog(input: { action: string; actorId?: string | null; actorName?: string | null; guildId: string; metadata?: Record<string, unknown>; recordId?: string | null }) { await this.http.post("/police-qru/bot/logs", input); }
  async getPolicePromotionSettings(guildId: string) { const { data } = await this.http.get<{ settings: PolicePromotionSettings }>(`/police-promotions/bot/${guildId}/settings`); return data.settings; }
  async createPolicePromotionRequest(input: { answers: PolicePromotionAnswer[]; guildId: string; previousRequestId?: string | null; promotionId: string; requesterId: string; requesterName: string }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>("/police-promotions/bot/requests", input); return data.request; }
  async clonePolicePromotionRequest(requestId: string, input: { actorId?: string | null; actorName?: string | null }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/new-evaluation`, input); return data.request; }
  async getPolicePromotionRequest(requestId: string) { const { data } = await this.http.get<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}`); return data.request; }
  async getPolicePromotionRequestByChannel(channelId: string) { const { data } = await this.http.get<{ request: PolicePromotionRequest | null }>(`/police-promotions/bot/channels/${channelId}/request`); return data.request; }
  async updatePolicePromotionTicketState(requestId: string, input: { channelId?: string | null; channelMessageId?: string | null; logChannelId?: string | null }) { const { data } = await this.http.patch<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/ticket`, input); return data.request; }
  async assignPolicePromotionEvaluator(requestId: string, input: { evaluatorId: string; evaluatorName: string }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/assign`, input); return data.request; }
  async finishPolicePromotionEvaluation(requestId: string, input: { evaluatorId: string; evaluationNotes: string; evaluationResult: "approved" | "rejected" }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/evaluation`, input); return data.request; }
  async listPolicePromotionEvaluationHistory(guildId: string, input: { cadetId?: string | null; limit?: number; page?: number }) { const { data } = await this.http.get<PolicePromotionEvaluationHistoryPage>(`/police-promotions/bot/${guildId}/history`, { params: input }); return data; }
  async updatePolicePromotionApprovalMessage(requestId: string, input: { approvalChannelId?: string | null; approvalMessageId?: string | null }) { const { data } = await this.http.patch<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/approval-message`, input); return data.request; }
  async decidePolicePromotionRequest(requestId: string, input: { actorId: string; actorName: string; approvalReason?: string | null; result: "approved" | "rejected" }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/decision`, input); return data.request; }
  async closePolicePromotionRequest(requestId: string, input: { actorId?: string | null; actorName?: string | null; status?: "cancelled" | "closed" }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/close`, input); return data.request; }
  async addPolicePromotionHistory(requestId: string, input: { action: string; actorId?: string | null; actorName?: string | null; metadata?: Record<string, unknown> }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/history`, input); return data.request; }
  ...
  async listVisibleMessageUsers(guildId: string) { const { data } = await this.http.get<{ users: VisibleMessageUser[] }>(`/visible-message/bot/${guildId}/users`); return data.users; }
  async isVisibleMessageUserEnabled(guildId: string, userId: string) { const { data } = await this.http.get<{ enabled: boolean }>(`/visible-message/bot/${guildId}/users/${userId}/enabled`); return data.enabled; }
  ...
  async updateZtkRankingMessageState(guildId: string, clanId: string, input: { channelId: string | null; kind: "online" | "ranking" | "recruitment"; messageId: string | null }) {
