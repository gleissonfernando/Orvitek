...
fivemActionsRouter.patch("/bot/:guildId/:architecture/settings", requireBot, async (req, res, next) => { try { const { guildId, architecture } = params(req); const botId = await botRuntimeId(req); await licensed(botId, architecture); res.json({ settings: await saveFivemActionSettings(botId, guildId, architecture, settingsSchema.parse(req.body), req.get("x-actor-id") ?? null) }); } catch (error) { next(error); } });
fivemActionsRouter.post("/bot/:guildId/:architecture/publish", requireBot, async (req, res, next) => { try { const { guildId, architecture } = params(req); const botId = await botRuntimeId(req); await licensed(botId, architecture); res.json({ settings: await requestFivemActionPanel(botId, guildId, architecture, req.get("x-actor-id") ?? "bot-runtime") }); } catch (error) { next(error); } });
