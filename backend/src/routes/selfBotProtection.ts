...
const settingsSchema = z.object({
  ...
  ignoredUserIds: z.array(snowflakeSchema).max(250).optional(),
  ignoredRoleIds: z.array(snowflakeSchema).max(250).optional(),
  ignoredBotIds: z.array(snowflakeSchema).max(250).optional(),
  ignoredCategoryIds: z.array(snowflakeSchema).max(250).optional(),
  ...
  blockGifs: z.boolean().optional(),
  blockVideos: z.boolean().optional(),
  ...
  stickerLimit: z.coerce.number().int().min(1).max(50).optional(),
  stickerWindowSeconds: z.coerce.number().int().min(1).max(3_600).optional(),
  nicknameChangeLimit: z.coerce.number().int().min(1).max(50).optional(),
  nicknameWindowSeconds: z.coerce.number().int().min(1).max(3_600).optional(),
  antiBotAction: z.enum(["allow", "kick", "ban", "manual"]).optional(),
  raidLockdownEnabled: z.boolean().optional(),
  dmWarningEnabled: z.boolean().optional(),
  dmWarningMessage: z.string().max(1_500).optional(),
  moduleLogChannelIds: z.record(moduleIdSchema, snowflakeSchema).optional(),
