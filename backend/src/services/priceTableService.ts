...
function defaultPriceTable(): Omit<MongoPriceTable, "_id" | "botId" | "createdAt" | "createdBy" | "guildId" | "messageId" | "updatedAt" | "updatedBy"> {
  return {
    ...
    currency: "BRL",
    currencyFormat: "R$",
    description: "Aqui você encontra os valores dos sistemas disponíveis para contratacao.",
    ...
    items: [
      {
        ...
        description: "Bot básico com comandos e painel inicial.",
        highlight: false,
        id: randomUUID(),
        name: "Bot Simples",
        order: 0,
    ...
    panelSections: {
      ...
      includedItems: ["Painel completo pela Dashboard", "Atualizacoes e configuração sem alterar código"],
      systemsTitle: "SISTEMAS",
      systemsText: "**FAC**\n• Ausências\n• Hierarquia\n• Ações\n• Financeiro\n• Encomendas e metas",
      advantagesTitle: "VANTAGENS",
      advantages: ["Configuração completa pela Dashboard", "Atualização em tempo real", "Suporte especializado"],
...
async function writePriceTableLog(action: string, table: Pick<MongoPriceTable, "_id" | "botId" | "guildId" | "logChannelId" | "name">, actorId: string | null, data: Record<string, unknown> = {}) {
  ...
  await createLog({
    ...
    message: `Tabela de preços ${table.name}: ${action}.`,
