...
export function DashboardHome(props: DashboardHomeProps) {
  ...
  return (
    <div className="nex-tech-home -mx-4 overflow-hidden px-4 pb-8 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
      <section className="relative isolate overflow-hidden rounded-[1.75rem] border border-[#FFD500]/20 bg-[#070707] px-5 py-12 shadow-[0_35px_100px_rgba(0,0,0,.45)] sm:px-10 sm:py-16 xl:px-14">
        <GlowBackground />
        <div className="relative grid items-center gap-12 xl:grid-cols-[1.02fr_.98fr]">
          <motion.div initial={reducedMotion ? false : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65 }}>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#FFD500]/30 bg-[#FFD500]/10 px-3 py-1.5 text-xs font-medium text-[#FFEA70]">
            ...
            <h1 className="mt-6 max-w-3xl text-4xl font-bold tracking-[-.045em] text-white sm:text-5xl xl:text-6xl">
              Transforme sua operação com a <span className="text-[#FFEA70]">tecnologia da Nex Tech</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-xs text-slate-400">
              {["Acesso centralizado", "Visibilidade em tempo real", "Configuração por módulos"].map((item) => (
                <span className="flex items-center gap-2" key={item}><CheckCircle2 className="h-4 w-4 text-[#FFD500]" />{item}</span>
              ))}
            </div>
          </motion.div>
      ...
      <section className="home-section grid items-center gap-10 xl:grid-cols-2">
        <div>
          <SectionHeading align="left" eyebrow="Integrações" title="Conecte as peças da sua operação" text="A Nex Tech reúne diferentes pontos de contato em um fluxo visual e centralizado." />
          <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3">
            {integrations.map(({ icon: ItemIcon, label }) => <div className="flex items-center gap-3 rounded-xl border border-[#FFD500]/15 bg-[#FFD500]/[.04] p-3 text-sm text-zinc-300" key={label}><ItemIcon className="h-4 w-4 text-[#FFEA70]" />{label}</div>)}
          </div>
        </div>
        <IntegrationVisual />
      </section>
      ...
      <section className="home-section grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
        <Reveal className="rounded-[1.75rem] border border-[#FFD500]/20 bg-[#090909] p-7 sm:p-9">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-[#FFD500]/30 bg-[#FFD500]/10"><ShieldCheck className="h-7 w-7 text-[#FFEA70]" /></div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">{["Proteção de sessões", "Controle de acesso", "Logs de atividades", "Validação de requisições", "Permissões por função", "Monitoramento contínuo"].map(item => <span className="flex items-center gap-2 text-sm text-slate-300" key={item}><Check className="h-4 w-4 text-[#FFD500]" />{item}</span>)}</div>
        </Reveal>
        <Reveal className="rounded-[1.75rem] border border-white/[.07] bg-[#090914] p-5 sm:p-7">
          <div className="flex items-center justify-between border-b border-white/[.07] pb-4"><div><p className="text-sm font-semibold text-white">Centro de proteção</p><p className="mt-1 text-xs text-slate-500">Visão operacional</p></div><span className="rounded-full bg-[#FFD500]/10 px-3 py-1 text-xs text-[#FFEA70]">Monitorado</span></div>
          <div className="mt-5 space-y-3">{[[KeyRound,"Sessão autenticada","Acesso verificado"],[ShieldCheck,"Escopo do servidor","Contexto isolado"],[LockKeyhole,"Credenciais privadas","Mantidas no backend"]].map(([ItemIcon,title,text]) => { const C = ItemIcon as Icon; return <div className="flex items-center gap-4 rounded-xl border border-white/[.06] bg-white/[.025] p-4" key={String(title)}><C className="h-5 w-5 text-[#FFEA70]"/><div><p className="text-sm font-medium text-white">{String(title)}</p><p className="mt-1 text-xs text-slate-500">{String(text)}</p></div></div>})}</div>
        </Reveal>
      </section>

      {featuredModules.length ? <section className="home-section"><SectionHeading eyebrow="Seu ambiente" title="Módulos disponíveis" text="Acesse rapidamente as ferramentas liberadas para este bot." /><div className="mt-9 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{featuredModules.map(({ icon: ModuleIcon, ...module }) => <button className="group flex min-h-32 items-start gap-4 rounded-2xl border border-white/[.07] bg-white/[.025] p-5 text-left transition hover:-translate-y-1 hover:border-[#FFD500]/30 hover:bg-[#FFD500]/[.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FFEA70]" key={module.id} onClick={module.onOpen} type="button"><span className="rounded-xl bg-[#FFD500]/10 p-2.5 text-[#FFEA70]"><ModuleIcon className="h-5 w-5" /></span><span><strong className="text-sm text-white">{module.title}</strong><span className="mt-2 block text-xs leading-5 text-slate-500">{module.description}</span></span><ArrowRight className="ml-auto h-4 w-4 text-slate-600 transition group-hover:translate-x-1 group-hover:text-[#FFEA70]" /></button>)}</div></section> : null}
...
function GlowBackground() { return <div aria-hidden className="pointer-events-none absolute inset-0"><div className="absolute -left-32 -top-40 h-96 w-96 rounded-full bg-[#FFD500]/15 blur-3xl"/><div className="absolute -right-28 top-10 h-80 w-80 rounded-full bg-[#FFEA70]/10 blur-3xl"/><div className="nex-tech-grid absolute inset-0 opacity-30"/></div> }

function PlatformPreview(props: DashboardHomeProps) { return <motion.div className="relative" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .7, delay: .15 }}><div className="absolute -inset-4 rounded-[2rem] bg-[#FFD500]/10 blur-2xl"/><div className="relative overflow-hidden rounded-2xl border border-[#FFD500]/15 bg-[#090909]/95 shadow-2xl"><div className="flex items-center gap-2 border-b border-white/[.07] px-4 py-3"><i className="h-2.5 w-2.5 rounded-full bg-zinc-500/70"/><i className="h-2.5 w-2.5 rounded-full bg-amber-300/80"/><i className="h-2.5 w-2.5 rounded-full bg-[#FFD500]/80"/><span className="ml-2 text-[10px] text-slate-600">nex-tech / operação</span></div><div className="grid grid-cols-[62px_1fr] sm:grid-cols-[88px_1fr]"><div className="border-r border-white/[.06] p-3"><div className="mx-auto h-8 w-8 rounded-lg bg-[#FFD500]"/><div className="mt-6 space-y-3">{[1,2,3,4,5].map(i=><div className={`mx-auto h-7 rounded-md ${i===1 ? "bg-[#FFD500]/15" : "bg-white/[.04]"}`} key={i}/>)}</div></div><div className="min-w-0 p-4 sm:p-5"><div className="flex items-center justify-between"><div><p className="text-xs text-slate-500">Servidor atual</p><p className="mt-1 max-w-40 truncate text-sm font-semibold text-white">{props.guildName}</p></div><span className={`rounded-full px-2 py-1 text-[10px] ${props.botOnline ? "bg-[#FFD500]/10 text-[#FFEA70]" : "bg-slate-400/10 text-slate-400"}`}>{props.botOnline ? "Online" : "Offline"}</span></div><div className="mt-5 grid grid-cols-2 gap-2"><MiniMetric label="Membros" value={compact(props.memberCount)}/><MiniMetric label="Módulos" value={`${props.activeModules}/${props.totalModules}`}/></div><div className="mt-3 rounded-xl border border-white/[.06] bg-white/[.02] p-3"><div className="flex items-end gap-1.5">{[32,48,40,68,54,78,62,88,72,92,84,96].map((height,i)=><motion.i className="flex-1 rounded-t bg-gradient-to-t from-[#FFD500]/25 to-[#FFEA70]/85" initial={{height:4}} animate={{height}} transition={{duration:.5,delay:.25+i*.035}} key={i}/>)}</div><div className="mt-3 flex justify-between text-[9px] text-slate-600"><span>Atividade</span><span>Tempo real</span></div></div><div className="mt-3 space-y-2">{["Serviços sincronizados","Permissões verificadas"].map((item,i)=><div className="flex items-center gap-2 rounded-lg bg-white/[.025] px-3 py-2 text-[10px] text-slate-400" key={item}><span className={`h-1.5 w-1.5 rounded-full ${i ? "bg-[#FFEA70]" : "bg-[#FFD500]"}`}/>{item}</div>)}</div></div></div></div></motion.div> }
...
function TrustItem({icon:Icon,title,text}:{icon:Icon;title:string;text:string}) { return <div className="flex items-center gap-3 rounded-2xl border border-white/[.07] bg-white/[.025] p-4"><span className="rounded-xl bg-[#FFD500]/10 p-2.5 text-[#FFEA70]"><Icon className="h-5 w-5"/></span><div><p className="text-sm font-medium text-white">{title}</p><p className="mt-1 text-xs text-slate-500">{text}</p></div></div> }
function SectionHeading({align="center",eyebrow,title,text}:{align?:"left"|"center";eyebrow:string;title:string;text:string}) { return <Reveal className={align === "center" ? "mx-auto max-w-2xl text-center" : "max-w-xl"}><p className="text-xs font-semibold uppercase tracking-[.25em] text-[#FFEA70]">{eyebrow}</p><h2 className="mt-4 text-3xl font-bold tracking-tight text-white sm:text-4xl">{title}</h2><p className="mt-4 leading-7 text-slate-400">{text}</p></Reveal> }
function SolutionCard({icon:Icon,title,text,index}:{icon:Icon;title:string;text:string;index:number}) { return <Reveal delay={index*.05} className="group rounded-2xl border border-white/[.07] bg-gradient-to-br from-white/[.04] to-transparent p-6 transition duration-300 hover:-translate-y-1 hover:border-[#FFD500]/25 hover:shadow-[0_20px_60px_rgba(255,213,0,.12)]"><span className="inline-flex rounded-xl border border-[#FFD500]/20 bg-[#FFD500]/10 p-3 text-[#FFEA70]"><Icon className="h-5 w-5"/></span><h3 className="mt-5 font-semibold text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{text}</p></Reveal> }
function Feature({icon:Icon,title,text,bullets}:{icon:Icon;title:string;text:string;bullets:string[]}) { return <Reveal className="rounded-[1.5rem] border border-white/[.07] bg-[#0a0a0a] p-6 sm:p-7"><Icon className="h-7 w-7 text-[#FFEA70]"/><h3 className="mt-6 text-xl font-semibold text-white">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-400">{text}</p><ul className="mt-6 space-y-3">{bullets.map(item=><li className="flex items-center gap-2 text-sm text-slate-300" key={item}><Check className="h-4 w-4 text-[#FFD500]"/>{item}</li>)}</ul></Reveal> }
...
function IntegrationVisual() { return <Reveal className="relative mx-auto flex min-h-80 w-full max-w-xl items-center justify-center overflow-hidden rounded-[1.75rem] border border-white/[.07] bg-[#090909]"><div className="nex-tech-grid absolute inset-0 opacity-25"/><div className="absolute h-56 w-56 rounded-full border border-[#FFD500]/15"/><div className="absolute h-40 w-40 rounded-full border border-[#FFEA70]/15"/>{integrations.map(({icon:Icon,label},i)=>{const angle=(Math.PI*2*i)/integrations.length;const y=Math.sin(angle)*105;return <motion.div className="absolute flex h-12 w-12 items-center justify-center rounded-xl border border-[#FFD500]/15 bg-[#101010] text-[#FFEA70] shadow-lg" style={{x:Math.cos(angle)*105}} animate={{y:[y,y-5,y]}} transition={{duration:3+i*.2,repeat:Infinity,ease:"easeInOut"}} title={label} key={label}><Icon className="h-5 w-5"/></motion.div>})}<div className="relative flex h-20 w-20 items-center justify-center rounded-2xl bg-[#FFD500] shadow-[0_0_45px_rgba(255,213,0,.25)]"><Globe2 className="h-9 w-9 text-black"/></div></Reveal> }
