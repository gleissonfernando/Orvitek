...
export function DmBarPanel({ botId, canManage, guild }: { botId?: string | null; canManage: boolean; guild: DashboardGuild | null }) {
  ...
  const patch = (next: Partial<DmBarConfig>) => setData((current) => current ? { ...current, config: { ...current.config, ...next } } : current);

  async function save() {
    setSaving(true); setMessage(null);
    try { const next = await saveDmBarConfig(guild!.id, botId!, config); setData((current) => current ? { ...current, config: next } : current); setMessage("Configurações salvas com sucesso."); }
  ...
  async function reset() {
    ...
    setSaving(true);
    try { const next = await resetDmBarConfig(guild!.id, botId!); setData((current) => current ? { ...current, config: next } : current); }
    catch (error) { setMessage(readMessage(error)); }
    finally { setSaving(false); }
  }
  async function upload(kind: "main" | "footer", file: File | null) {
    if (!file) return;
    setSaving(true); setMessage(null);
    try { const next = await uploadDmBarImage(guild!.id, botId!, kind, file); setData((current) => current ? { ...current, config: next } : current); setMessage("Imagem enviada."); }
    catch (error) { setMessage(readMessage(error)); }
    finally { setSaving(false); }
  }
  async function removeImage(kind: "main" | "footer") {
    setSaving(true);
    try { const next = await removeDmBarImage(guild!.id, botId!, kind); setData((current) => current ? { ...current, config: next } : current); }
    catch (error) { setMessage(readMessage(error)); }
    finally { setSaving(false); }
  }

  return <div className="space-y-5">
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><Send className="h-5 w-5 text-emerald-300" />Barra DM</CardTitle><CardDescription>Envio de mensagens privadas com painel visual, permissões, imagens e logs.</CardDescription></CardHeader></Card>
    ...
    <Card><CardHeader><CardTitle>Mensagem da DM</CardTitle><CardDescription>Variáveis: {"{usuario}"}, {"{usuario_nome}"}, {"{autor}"}, {"{servidor}"}, {"{data}"}, {"{hora}"}, {"{mensagem}"}.</CardDescription></CardHeader><CardContent className="grid gap-4 lg:grid-cols-2">
    ...
    <Card><CardHeader><CardTitle>Imagens e rodapé</CardTitle></CardHeader><CardContent className="grid gap-4 lg:grid-cols-2">
      ...
      <label className="flex items-center justify-between rounded-lg border border-zinc-800 p-3 text-sm text-white">Mostrar autor<Switch checked={config.showSender} disabled={!canManage} onCheckedChange={(showSender) => patch({ showSender })} /></label>
...
function Preview({ config, guildName }: { config: DmBarConfig; guildName: string }) { const text = applyVars(config.descriptionTemplate, guildName); return <div className="rounded-lg border border-zinc-800 bg-[#101114] p-4"><p className="text-lg font-bold text-white">{applyVars(config.titleTemplate, guildName)}</p>{config.mainImageUrl ? <img className="my-3 max-h-40 rounded-md object-cover" src={config.mainImageUrl} /> : null}<p className="whitespace-pre-wrap text-sm text-zinc-300">{text}</p>{config.footerEnabled ? <div className="mt-4 border-t border-zinc-700 pt-3 text-xs text-zinc-500">{config.footerIconUrl ? <img className="mr-2 inline h-6 w-6 rounded" src={config.footerIconUrl} /> : null}{applyVars(config.footerText, guildName)}</div> : null}</div>; }
function applyVars(value: string, guildName: string) { return value.replaceAll("{usuario}", "@Usuario").replaceAll("{usuario_nome}", "Usuario").replaceAll("{usuario_nick}", "Usuario").replaceAll("{autor}", "@Autor").replaceAll("{autor_nome}", "Autor").replaceAll("{servidor}", guildName).replaceAll("{data}", new Date().toLocaleDateString("pt-BR")).replaceAll("{hora}", new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })).replaceAll("{mensagem}", "Mensagem de exemplo da Barra DM.").replaceAll("{id_usuario}", "123456789").replaceAll("{id_autor}", "987654321"); }
