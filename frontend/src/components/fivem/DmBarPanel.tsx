...
export function DmBarPanel({ botId, canManage, guild }: { botId?: string | null; canManage: boolean; guild: DashboardGuild | null }) {
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>Mensagem da DM</CardTitle><CardDescription>Variáveis: {"{usuário}"}, {"{usuario_nome}"}, {"{servidor}"}, {"{data}"}, {"{hora}"}, {"{mensagem}"}.</CardDescription></CardHeader><CardContent className="grid gap-4 lg:grid-cols-2">
